...
bool ok(int l, int n, int m, vvi grid){
    ...
    for(int i = 1; i <= n - l + 1; ++i){
        for(int j = 1; j <= m - l + 1; ++j){
            int sum = prefix[i + l - 1][j + l - 1] - prefix[i - 1][j + l - 1] - prefix[i + l - 1][j - 1] + prefix[i - 1][j - 1];
            if(sum == l*l) return false;
        }
    }
    return true;
}
