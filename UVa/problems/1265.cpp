...
ll solve(){
    ...
    for(auto [key, value] : mp){
        ...
        for(auto [u, v] : value){
            ...
            if(has.find(ufds.findSet(u)) != has.end()) ans -= ufds.sizeOfSet(u);
            if(has.find(ufds.findSet(v)) != has.end()) ans -= ufds.sizeOfSet(v);
            ufds.unionSet(u, v);
            ans += ufds.sizeOfSet(u);
            has.insert(ufds.findSet(u));
        }
    }
    return ans;
}
