...
int solve(){
    int r, c;
    cin >> r >> c;
    vvi grid(r, vi(c + 2, -1)), cost(r, vi(c + 2, INF)), vis(r, vi(c + 2));
    for(int i = 0; i < r; ++i){
        for(int j = 1; j <= c; ++j){
            cin >> grid[i][j];
        }
    ...
    while(!pq.empty()){
        ...
        if(vis[i][j]) continue; // Doesnt cause that much speedup since only 4 neighbours
        for(int d = 0; d < 4; ++d){
            int x = i + dr[d], y = j + dc[d];
            if(x < 0 || x > r - 1 || y < 0 || y > c + 1) continue;
            int val = max(cost[i][j], grid[x][y]);
            if(val < cost[x][y]){
                cost[x][y] = val;
                pq.push({-val, x, y});
            }
